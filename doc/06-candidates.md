...
- 시험날짜는 필수값이며 `yyyy-mm-dd` 형식만 허용한다. 예: `2026-03-28`.
- 생년월일은 필수값이며 `yyyy-mm-dd` 형식만 허용한다. 예: `2006-01-02`.
- 시작시간은 필수값이며 `hh:mm` 형식만 허용한다. 예: `08:40`.
- 종료시간은 선택값이며 값이 있으면 `hh:mm` 형식만 허용한다. 예: `10:00`.
- 업로드 양식은 시험날짜, 생년월일, 시작시간, 종료시간 헤더 셀에 입력 형식 주석을 제공한다.
- 모든 입력된 XLSX cell은 text 형식이어야 한다.
- 수험번호와 교시코드는 필수이며, 두 값을 조합한 키가 중복 기준이다.
- XLSX 내부에서 같은 수험번호와 교시코드 조합이 중복되면 reject.
