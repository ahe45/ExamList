...
function createCandidateWorkbookService({ createHttpError }) {
  ...
  function normalizeCandidateWorkbookInput(candidateInput = {}, index = -1, options = {}) {
    ...
    return {
      ...
      opt6: normalizeOptionalText(candidateInput.opt6 ?? candidateInput.OPT6),
      opt7: normalizeOptionalText(candidateInput.opt7 ?? candidateInput.OPT7),
      opt8: normalizeOptionalText(candidateInput.opt8 ?? candidateInput.OPT8),
  ...
  function normalizeCandidateExportRow(record = {}) {
    return {
      ...
      opt6: String(record.opt6 ?? "").trim(),
      opt7: String(record.opt7 ?? "").trim(),
      opt8: String(record.opt8 ?? "").trim(),
      opt9: String(record.opt9 ?? "").trim(),
      opt10: String(record.opt10 ?? "").trim(),
