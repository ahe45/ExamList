...
function toCandidateWorkbookRow(row = {}) {
  return {
    ...
    admissionYear: String(row.admissionYear || row.admission_year || row.admissionYearName || "").trim(),
    admissionCode: String(row.admissionCode || row.applicationNo || "").trim(),
    birth: formatDateValue(row.birthDate || row.birth || ""),
...
function mapCandidateViewRow(row = {}) {
  ...
  const examDate = formatDateValue(row.examDate);
  const birthDate = formatDateValue(row.birthDate);
