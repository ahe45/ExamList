...
function normalizeCandidateFilter(rawFilter = {}) {
  return {
    ...
    opt6: String(rawFilter.opt6 || "").trim(),
    opt7: String(rawFilter.opt7 || "").trim(),
    opt8: String(rawFilter.opt8 || "").trim(),
    opt9: String(rawFilter.opt9 || "").trim(),
    opt10: String(rawFilter.opt10 || "").trim(),
...
function normalizeCandidateRecordInput(input = {}, sourceType = "manual", schoolId = "school-default") {
  ...
  const record = {
    ...
    opt6: normalizeText(raw.opt6 ?? raw.OPT6 ?? input.opt6, 255),
    opt7: normalizeText(raw.opt7 ?? raw.OPT7 ?? input.opt7, 255),
    opt8: normalizeText(raw.opt8 ?? raw.OPT8 ?? input.opt8, 255),
    opt9: normalizeText(raw.opt9 ?? raw.OPT9 ?? input.opt9, 255),
    opt10: normalizeText(raw.opt10 ?? raw.OPT10 ?? input.opt10, 255),
...
function toCandidateWorkbookRow(row = {}) {
  return {
    ...
    opt6: String(row.opt6 || ""),
    opt7: String(row.opt7 || ""),
    opt8: String(row.opt8 || ""),
    opt9: String(row.opt9 || ""),
    opt10: String(row.opt10 || ""),
