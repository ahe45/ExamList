...
function renderDetailLoading() {
  return '<p class="helper-text">생성 이력 상세 정보를 불러오는 중입니다.</p>';
}

function renderDetailEmpty(errorMessage = "") {
  return `
    <p class="${errorMessage ? "error-banner" : "helper-text"}">
      ${escapeHtml(errorMessage || "표시할 생성 이력이 없습니다.")}
    </p>
  `;
}

function renderPdfGenerationDetailModalBody(item = {}, access = null) {
  return `
...
export function renderPdfGenerationDetailModal({ access, detail, pdfGenerations }) {
  const modal = pdfGenerations.detailModal || {};
  ...
  const title = item?.fileName || "PDF 생성 결과 상세";

  return `
    <div class="modal-overlay pdf-generation-detail-overlay" role="dialog" aria-modal="true" aria-labelledby="pdfGenerationDetailModalTitle">
      <div class="modal-card pdf-generation-detail-modal-card">
        <div class="modal-header">
          <div>
            <p class="section-kicker">생성 이력 상세</p>
            <h2 id="pdfGenerationDetailModalTitle">${escapeHtml(title)}</h2>
          </div>
          <button class="icon-button" data-action="close-pdf-generation-detail-modal" type="button" aria-label="닫기">×</button>
        </div>
        <div class="pdf-generation-detail-modal-body">
          ${
            detail.loading
              ? renderDetailLoading()
              : item
                ? renderPdfGenerationDetailModalBody(item, access)
                : renderDetailEmpty(detail.errorMessage)
          }
        </div>
        <div class="modal-actions">
          <button class="ghost-button" data-action="close-pdf-generation-detail-modal" type="button">닫기</button>
        </div>
      </div>
    </div>
  `;
